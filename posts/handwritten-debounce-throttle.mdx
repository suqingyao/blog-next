...
---
  id: handwritten-debounce-throttle
  title: 手写防抖节流
  createdTime: 2024-05-26 22:24:54
  published: true
---
...
理解：如果你玩过Data类游戏的话，在游戏中节流就相当于释放了一个技能后，技能进入了冷却CD，在CD冷却完成之前是没法再次释放技能的，而防抖就是你一直按回城技能，只会以最后一次为准开始计时。（现在是云玩家了，印象中是这样的机制）
...
实现原理：如果未定义定时器就创建定时器，执行完定时器回调函数时再将定时器清空并执行真正的handler
